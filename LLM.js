...
export const configPrompt = (name, type, description, techs)=>`${SystemPrompt(name, type)} The project description is: ${description}. This is the list of technolgies that are going to be used: ${techs.join(', ')}. Determine what configuration files are needed and what dependencies you need to install. ${JSONFormater}`;
...
async function installNPMPackage(packageName, opts = {
    ...
    return new Promise((resolve, reject) => {
        ...
        exec(command, (error, stdout, stderr) => {
            if (error) {
                console.error(chalk.red(`Error installing package ${packageName}: ${error.message}`));
                reject(error);
            } else {
    ...
    while(stepInference.choices[0].message?.tool_calls?.length > 0 || !done){
        for(const toolCall of stepInference.choices[0].message.tool_calls){
            ...
            try {
                ...
                stepInference.choices[0].message.tool_calls = stepInference.choices[0].message.tool_calls.filter(call => call.function.name !== funcName);
